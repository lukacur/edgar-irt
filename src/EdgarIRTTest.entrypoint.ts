...
export class EdgarIRTTestEntrypoint {
    ...
    private static async doChildProcTest(): Promise<void> {
        ...
        const fetchTimeoutId = TimeoutUtil.doTimeout(
            EdgarIRTTestEntrypoint.DEFAULT_TIMEOUT_MS,
            () => {
    ...
    private static async doDaemonTest(args: string[]): Promise<void> {
        ...
        await EdgarIRTTestEntrypoint.daemonQueue.enqueue({
            idJobType: 1,
    ...
    private static async doDelayedPromiseTest(): Promise<void> {
        ...
        EdgarIRTTestEntrypoint.delayableAwaiter(delayedPromise);
    ...
    private createDefaultConnection(...args: any[]): object {
        if (EdgarIRTTestEntrypoint.defaultConnection === null) {
            throw new Error("Default connection was not set up");
        }

        return EdgarIRTTestEntrypoint.defaultConnection;
    }*/

    private static readonly CURRENT_TEST: AvailableTests = "daemon";

    public static async main(args: string[]): Promise<void> {
        // EdgarIRTTestEntrypoint.defaultConnection = await DatabaseConnection.fromConfigFile("./database-config.json");
        ...
        switch (EdgarIRTTestEntrypoint.CURRENT_TEST) {
            case "db": {
                prom = EdgarIRTTestEntrypoint.doDbTest(conn);
                break;
            }

            case "child_process": {
                prom = EdgarIRTTestEntrypoint.doChildProcTest();
                break;
            }

            case "stats_processor": {
                prom = EdgarIRTTestEntrypoint.doStatsProcTest(conn);
                break;
            }

            case "delayed_promise": {
                prom = EdgarIRTTestEntrypoint.doDelayedPromiseTest();
                break;
            }

            case "serialization": {
                prom = EdgarIRTTestEntrypoint.doSerializationTest(conn);
                break;
            }

            case "daemon": {
                prom = EdgarIRTTestEntrypoint.doDaemonTest(args);
                break;
            }

            case "service_setup": {
                prom = Promise.resolve(); // EdgarIRTTestEntrypoint.doServiceSetupTest();
                break;
            }

            case "queue": {
                prom = EdgarIRTTestEntrypoint.doQueueTests();
                break;
            }

            case "job": {
                prom = EdgarIRTTestEntrypoint.doJobsTest(conn);
                break;
            }

            case "total_job": {
                prom = EdgarIRTTestEntrypoint.doTotalJobTest(conn);
                break;
            }

            case "generic_job": {
                prom = EdgarIRTTestEntrypoint.doGenericJobTest(conn);
                break;
            }

            case "dynamic_imports": {
                prom = EdgarIRTTestEntrypoint.doDynamicImportsTest();
                break;
            }
